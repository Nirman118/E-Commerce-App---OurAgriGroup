...
class AgriLab extends Component {
    constructor(props) {
        super(props);
        this.state = {
            user: '',
        ...
        this.ref= firestore().collection('agrilab')
        this.getInfo = this.getInfo.bind(this)

    }
    async componentDidMount() {
        this.ref.onSnapshot(this.getInfo)
        console.log("hi")
    ...
    async getInfo(querySnapshot){
        const info = [];
        if(querySnapshot){
          querySnapshot.forEach(async(doc) => {
            const{ email,  village, Firstname,Lastname, age, phoneno, state, city,pincode, Lastdate,Lastdateavailable,startdate,startdateavailable} = doc.data();
            info.push({
              key: doc.id,
              firstname: Firstname,
              lastname:Lastname,
        ...
        this.setState({datas:info})
        console.log("infochhk", this.state.datas)
      }
    render() {
        const renderItem = (item) => {
            let k = item.item
            console.log("hii",k.village)
            return (
                <View style={{  flex:0.5, backgroundColor: "white" , margin:10, borderRadius:20,paddingBottom:10}}>
                <Text style={styles.billtxt} >Name: {k.firstname} {k.lastname}</Text>
                <Text style={styles.billtxt} >Age: {k.age}</Text>
                <Text style={styles.billtxt} >Phone number: {k.phoneno}</Text>
                <Text style={styles.billtxt} >Village: {k.village}</Text>
                <Text style={styles.billtxt} >City: {k.city}</Text>
                <Text style={styles.billtxt} >State: {k.state}</Text>
                <Text style={styles.billtxt} >Pincode: { k.pincode}</Text>
                <Text style={styles.billtxt} >Start date: { k.startdate}</Text>
                <Text style={styles.billtxt} >Start date Available: { k.startdateavailable}</Text>
                <Text style={styles.billtxt} >Last date: { k.lastdate}</Text>
                <Text style={styles.billtxt} >Last date Available: { k.Lastdateavailable}</Text>
                </View>
            )
        }
        ...
        return (
            <View style={{ justifyContent: "center", alignItems: "center", flex:1 }}>
                <ImageBackground source={require('../assets/screen2.png')} style={styles.screen2} >
                    ...
                    <Text style={{ fontWeight: "bold", fontSize: 26, textAlign: "center" }}>Agricultural Labourers</Text>
                    <ScrollView style={{flex:1}}>
                        <FlatList
                            data={this.state.datas}
                            keyExtractor={item => item.id}
                            renderItem={renderItem}
                            style={{  height: hp("100%"), weight: wp("100%"),flex:1 }}  
                            horizontal={false}
                        />
                        </ScrollView>
